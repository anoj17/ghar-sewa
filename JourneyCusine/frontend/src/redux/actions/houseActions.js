...
export const getAllListing = (param) => async (dispatch) => {
    try {

        const res = await api.get(`${API}house/get_all_listing?q=${param}`);
        if (res.status === 200) {
            dispatch({
                type: "ALL_LISTING",
                payload: res.data.allListingData
            })
        }
    } catch (error) {
        console.log('error0000000000000000000000000000',error)
