...
import { getAllListing } from "../redux/actions/houseActions";
...
const Home = () => {
  ...
  return (
    <main className="max-w-screen-2xl xl:px-10 px-5 sm:px-16 mx-auto">
      <section
        className={` pt-8 grid md:grid-cols-12 gap-5 bg-white sticky top-16 z-30 ${hasScroll ? " shadow" : " shadow-none"
          }`}
      >
        {/* categories */}
                  ...
                  data?.map((listing) => {
                    return (
                      ...
                      <Link
                        to={`/rooms/${listing?._id}`}
                        key={listing._id}
                        onClick={() => console.log('preview two')}
